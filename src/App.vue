    ...
    <div class="container text-center">
      <template v-if="domIsReady">
        ...
        <div class="mb-4 mt-4">
          ...
          />
          <button @click="goToUrl" class="btn btn-primary mt-3">
            <span
              v-if="loading"
        ...
        <p class="mt-4">
          ...
          <svg
            ...
            height="16"
            fill="currentColor"
            class="bi bi-exclamation-triangle status-warning"
            viewBox="0 0 16 16"
          >
            <path
        ...
        <p>
          ...
          <svg
            ...
            height="16"
            fill="currentColor"
            class="bi bi-speedometer2 status-warning"
            viewBox="0 0 16 16"
          >
            <path
        ...
        <button @click="toggleEnabled" class="btn btn-primary">
          <span v-if="status === 'Enabled'">Disable extension</span>
          <span v-else>Enable extension</span>
        </button>

        <button @click="toggleFastMode" class="btn btn-primary">
...
<script>
import { domIsReady } from "./utils/browser";
...
export default {
  ...
  mounted() {
    ...
    browser.storage.local.get(
      ["punkExtensionEnabled", "punkFastMode"],
      function (result) {
        if (
          result.punkExtensionEnabled === "Enabled" ||
          result.punkExtensionEnabled === "Disabled"
        ) {
          component.status = result.punkExtensionEnabled;
        } else {
          browser.storage.local.set(
            { punkExtensionEnabled: component.status },
            function () {}
          );
        }

        if (
          result.punkFastMode === "Fast" ||
          result.punkFastMode === "Normal"
        ) {
          component.mode = result.punkFastMode;
        } else {
          browser.storage.local.set(
            { punkFastMode: component.mode },
            function () {}
          );
        }
      }
